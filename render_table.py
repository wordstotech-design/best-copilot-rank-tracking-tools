...
The organizing idea of this list is not engine count, it is deployment shape.
Copilot sits inside Microsoft's own stack, so the split that matters is
whether a tool ties into that stack (tenant level reporting, SSO, an API a
data team can pull from) or only watches the public copilot.microsoft.com
chat surface for one login at a time. Edit one YAML entry and run this to
keep the table and the writeups in sync.
"""
...
COLUMNS = [
    ("name", "Tool"),
    ("copilot_coverage", "Copilot coverage"),
